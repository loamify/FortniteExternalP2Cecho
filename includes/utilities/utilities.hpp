...
enum offset {
	uworld = 0x11E88988, // need to change
	game_instance = 0x1d8, // sigma
	game_state = 0x160, // sigma
	...
	team_index = 0x1251, // sigma
	player_array = 0x2a8, // sigma
	pawn_private = 0x308, // sigma
	current_weapon = 0xa20, // sigma
	weapon_data = 0x4F0, // sigma
	velocity = 0x168, // sigma
	tier = 0x13b,
	is_cached = 0x45c,
	bone_array = 0x5B0, // need to change
	custom_time_dilation = 0x68, // sigma
...
namespace utilities {
	...
	public:
		...
		auto cursor_to(float x, float y) -> void {
			float AimSpeed = globals->smooth;
			FVector ScreenCenter = { (double)globals->width / 2 , (double)globals->height / 2, 0 };
			FVector Target;
			...
			Inject->set_cursor_position((int)Target.x, (int)Target.y);
		}
