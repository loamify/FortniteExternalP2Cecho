...
static std::string GetNameFromIndex(int key)
{
	...
	uint64_t NamePoolChunk = driver.read<uint64_t>(virtualaddy + FNAMEPOOL_OFFSET + (8 * ChunkOffset) + 16) + (unsigned int)(2 * NameOffset);
...
static std::string GetNameFromFName(int key)
{
	...
	uint64_t NamePoolChunk = driver.read<uint64_t>(virtualaddy + FNAMEPOOL_OFFSET + (8 * ChunkOffset) + 16) + (unsigned int)(2 * NameOffset); //((ChunkOffset + 2) * 8) ERROR_NAME_SIZE_EXCEEDED
...
uintptr_t RootComponent(uintptr_t actor)
{
	...
	return driver.read<uintptr_t>(actor + 0x198);
}
FVector GetLocation(uintptr_t actor)
{

	return driver.read<FVector>(RootComponent(actor) + 0x128);
}
...
namespace game {
	...
	public:
		...
		{
			float fLastSubmitTime = driver.read<float>(offset::skeletal_mesh + offset::fLastSubmitTime);
			float fLastRenderTimeOnScreen = driver.read<float>(offset::skeletal_mesh + offset::fLastRenderTimeOnScreen);
		...
		FVector LocationPredict(FVector Target, FVector Velocityy, float GetProjectileSpeed, float ProjectileGravity, float DistanceToTarget)
		{
			...
			Target.z += Velocityy.z * verticalTime +
				abs(-980.f * ProjectileGravity) * 0.5f * (verticalTime * verticalTime);
	...
	/*	void LevelDrawing()
		...
		static auto CacheData() -> void {
			...
			for (;; ) {
				...
				pointer->uworld = driver.read<uintptr_t>(virtualaddy + offset::uworld);
				if (Utilities->debug)
				{
					std::printf(" [uworld] -> %I64d\n", pointer->uworld);
				}
				pointer->game_instance = driver.read<uintptr_t>(pointer->uworld + offset::game_instance);
				if (Utilities->debug)
				{
					std::printf(" [game_instance] -> %I64d\n", pointer->game_instance);
				}
				pointer->levels = driver.read<uintptr_t>(pointer->levels + 0x170);
				if (Utilities->debug)
				{
					std::printf(" [levels] -> %I64d\n", pointer->levels);
				}
				pointer->game_state = driver.read<uintptr_t>(pointer->uworld + offset::game_state);
				if (Utilities->debug)
				{
					std::printf(" [game_state] -> %I64d\n", pointer->game_state);
				}
				pointer->local_player = driver.read<uintptr_t>(driver.read<uintptr_t>(pointer->game_instance + offset::local_player));
				if (Utilities->debug)
				{
					std::printf(" [local_player] -> %I64d\n", pointer->local_player);
				}
				pointer->player_controller = driver.read<uintptr_t>(pointer->local_player + offset::player_controller);
				if (Utilities->debug)
				{
					std::printf(" [player_controller] -> %I64d\n", pointer->player_controller);
				}
				pointer->acknowledged_pawn = driver.read<uintptr_t>(pointer->player_controller + offset::acknowledged_pawn);
				if (Utilities->debug)
				{
					std::printf(" [acknowledged_pawn] -> %I64d\n", pointer->acknowledged_pawn);
				}
				pointer->current_weapon = driver.read<uintptr_t>(pointer->acknowledged_pawn + offset::current_weapon);
				if (Utilities->debug)
				{
					std::printf(" [current_weapon] -> %I64d\n", pointer->current_weapon);
				}
				pointer->skeletal_mesh = driver.read<uintptr_t>(pointer->acknowledged_pawn + offset::skeletal_mesh);
				if (Utilities->debug)
				{
					std::printf(" [skeletal_mesh] -> %I64d\n", pointer->skeletal_mesh);
				}
				pointer->player_state = driver.read<uintptr_t>(pointer->acknowledged_pawn + offset::player_state);
				if (Utilities->debug)
				{
					std::printf(" [player_state] -> %I64d\n", pointer->player_state);
				}
				pointer->root_component = driver.read<uintptr_t>(pointer->acknowledged_pawn + offset::root_component);
				if (Utilities->debug)
				{
					std::printf(" [root_component] -> %I64d\n", pointer->root_component);
				}
				pointer->team_index = driver.read<int>(pointer->player_state + offset::team_index);
				if (Utilities->debug)
				{
					std::printf(" [team_index] -> %I64d\n", pointer->team_index);
				}
				pointer->relative_location = driver.read<FVector>(pointer->root_component + offset::relative_location);
				if (Utilities->debug)
				{
				...
				pointer->player_array = driver.read<uintptr_t>(pointer->game_state + offset::player_array);
				if (Utilities->debug)
				{
					std::printf(" [player_array] -> %I64d\n", pointer->player_array);
				}
				pointer->player_array_size = driver.read<int>(pointer->game_state + (offset::player_array + sizeof(uintptr_t)));
				if (Utilities->debug)
				{
				...
				for (int i = 0; i < pointer->player_array_size; ++i) {
					auto player_state = driver.read<uintptr_t>(pointer->player_array + (i * sizeof(uintptr_t)));
					auto udplayer_state = driver.read<uintptr_t>(pointer->player_array + (i * sizeof(uintptr_t)));

					auto current_actor = driver.read<uintptr_t>(player_state + offset::pawn_private);
					if (Utilities->debug)
					{
					...
					auto is_despawning = (driver.read<char>(current_actor + 0x758) >> 3);
					if (Utilities->debug)
					{
					...
					cached_actors.skeletal_mesh = driver.read<uintptr_t>(current_actor + offset::skeletal_mesh);
					cached_actors.root_component = pointer->root_component;
					cached_actors.player_state = player_state;
					cached_actors.team_index = pointer->team_index;
		...
		auto ActorLoop() -> void {
			...
			if (pointer->acknowledged_pawn) {
				...
				uint64_t player_weapon = driver.read<uint64_t>(pointer->acknowledged_pawn + 0xa20); // 	struct AFortWeapon* CurrentWeapon;
					...
					if (driver.is_valid(weapon_data)) {
						...
						if (driver.is_valid(ftext_ptr)) {
							...
							if (ftext_length > 0 && ftext_length < 50) {
								...
								driver.readm(reinterpret_cast<void*>(ftext_data), reinterpret_cast<uint8_t*>(ftext_buf), ftext_length * sizeof(wchar_t));
			...
			for (auto& cached_entity : entity_list) {
				...
				if (globals->triggerbot && GetAsyncKeyState_Spoofed(globals->triggerbot_key)) {
					POINT cursor;
					GetCursorPos(&cursor);
					if (Utilities->IsShootable(driver.read<FVector>(pointer->player_controller + offset::location_under_reticle), head_location)) {
				...
				if (globals->platform)
				{
					...
					if (globals->outline || globals->text_outline)
					{
						DrawString(globals->font_size, head_box.x - (text_size.x / 2), head_box.y - 18, visible_color_Text, false, true, platform_str.c_str());
					}
					else
					{
						DrawString(globals->font_size, head_box.x - (text_size.x / 2), head_box.y - 18, visible_color_Text, false, false, platform_str.c_str());
					}
				}

				if (globals->username) // name + platform
				{
					std::string username_str = Utilities->DecryptName(cached_entity.player_state);
					std::string platform_str = Utilities->platform(cached_entity.player_state);
					...
					if (globals->outline || globals->text_outline)
					{
						DrawString(globals->font_size, head_box.x - (textPosition.x / 2), head_box.y - 18, visible_color_Text, false, true, combinedText.c_str());
					}
					else
					{
						DrawString(globals->font_size, head_box.x - (textPosition.x / 2), head_box.y - 18, visible_color_Text, false, false, combinedText.c_str());
					}
				}
				...
				if (globals->head) {
					...
					if (globals->weapon_esp)
					{
						if (pointer->acknowledged_pawn)
						{
							...
							uint64_t player_weapon = driver.read<uint64_t>(cached_entity.actor + 0xa20);

							if (driver.is_valid(player_weapon)) {
								held_weapon.ammo_count = driver.read <int32_t>(player_weapon + 0xe1c);

								uint64_t weapon_data = driver.read <uint64_t>(player_weapon + 0x4f0);

								if (driver.is_valid(weapon_data)) {
									held_weapon.tier = driver.read <BYTE>(weapon_data + 0x13b);
									...
									if (driver.is_valid(ftext_ptr)) {
										...
										if (ftext_length > 0 && ftext_length < 50) {
											...
											driver.readm((PVOID)ftext_data, (PVOID)ftext_buf, ftext_length * sizeof(wchar_t));
								...
								if (globals->weapon_rarity)
								{
									if (WeaponInfo.tier == 0)
									{
										if (strstr(WeaponInfo.weapon_name.c_str(), E("Pickaxe")) != nullptr)
										{
											DrawString(15, (root_location.x) - (TextSize.x / 2), (root_location.y + add), ImColor(255, 255, 255), false, true, final.c_str());
										}
										else
										{
											std::string fina1l = ("Building Plan");
											ImVec2 TextSiz1e = ImGui::CalcTextSize(fina1l.c_str());
											DrawString(15, (root_location.x) - (TextSiz1e.x / 2), (root_location.y + add), ImColor(255, 255, 255), false, true, fina1l.c_str());
										}

									}
									if (WeaponInfo.tier == 1)
									{
										DrawString(15, (root_location.x) - (TextSize.x / 2), (root_location.y + add), ImColor(170, 165, 169), false, true, final.c_str());

									}
									if (WeaponInfo.tier == 2)
									{
										DrawString(15, (root_location.x) - (TextSize.x / 2), (root_location.y + add), ImColor(30, 255, 0), false, true, final.c_str());

									}
									if (WeaponInfo.tier == 3)
									{
										DrawString(15, (root_location.x) - (TextSize.x / 2), (root_location.y + add), ImColor(0, 112, 221), false, true, final.c_str());

									}
									if (WeaponInfo.tier == 4)
									{
										DrawString(15, (root_location.x) - (TextSize.x / 2), (root_location.y + add), ImColor(163, 53, 238), false, true, final.c_str());

									}
									if (WeaponInfo.tier == 5)
									{
										DrawString(15, (root_location.x) - (TextSize.x / 2), (root_location.y + add), ImColor(255, 128, 0), false, true, final.c_str());

									}
									if (WeaponInfo.tier == 6)
									{
										DrawString(15, (root_location.x) - (TextSize.x / 2), (root_location.y + add), ImColor(255, 255, 0), false, true, final.c_str());
								...
								else
								{
									if (WeaponInfo.tier == 0)
									{
										if (strstr(LocalPlayerWeapon.c_str(), E("Pickaxe")) != nullptr)
										{
											DrawString(15, (root_location.x) - (TextSize.x / 2), (root_location.y + add), visible_color, false, true, final.c_str());
										}
										else
										{
											std::string fina1l = ("Building Plan");
											ImVec2 TextSiz1e = ImGui::CalcTextSize(fina1l.c_str());
											DrawString(15, (root_location.x) - (TextSiz1e.x / 2), (root_location.y + add), visible_color, false, true, fina1l.c_str());
										}
									}
									else
									{
										DrawString(14, root_location.x - (text_size.x / 2), root_location.y + 20, visible_color, false, true, final.c_str());
									}
								}
							}
			...
			if (target_entity && globals->aimbot) {
				auto closest_mesh = driver.read<uint64_t>(target_entity + offset::skeletal_mesh);
				...
				if (globals->prediction) {
					if (EFortWeaponType::Sniper)
					{
						if (strstr(LocalPlayerWeapon.c_str(), ("Reaper Sniper Rifle"))) {
							...
							projectileGravityScale = 3.5f; // 3.5
							//projectileGravityScale = globals->projectileGravityScale;
							hitbox = LocationPredict(hitbox, Velocity, projectileSpeed, projectileGravityScale, Distance); //dmr LocationPredict  target_prediction CalculatePrediction
							std::printf(" [hitbox] -> %I64d\n", hitbox);
							std::printf(" [Velocity] -> %I64d\n", Velocity);
							std::printf(" [projectileSpeed] -> %I64d\n", projectileSpeed);
							std::printf(" [projectileGravityScale] -> %I64d\n", projectileGravityScale);
							std::printf(" [Distance] -> %I64d\n", Distance);
						}
						else if (strstr(LocalPlayerWeapon.c_str(), ("Huntress DMR")))
						{
							...
							auto Distance = camera::location.Distance(hitbox);
							std::cout << (skCrypt("Huntress DMR in HAND")) << std::flush;
							hitbox = LocationPredict(hitbox, Velocity, projectileSpeed, projectileGravityScale, Distance); //dmr LocationPredict  target_prediction CalculatePrediction
							std::printf(" [hitbox] -> %I64d\n", hitbox);
							std::printf(" [Velocity] -> %I64d\n", Velocity);
							std::printf(" [projectileSpeed] -> %I64d\n", projectileSpeed);
				...
				if (hitbox.x != 0 || hitbox.y != 0 && (Utilities->GetCrossDistance(hitbox.x, hitbox.y, globals->width / 2, globals->height / 2) <= globals->field_of_view))
				{
					...
					if (Utilities->IsVisible(closest_mesh)) {
						...
						if (globals->target_line) {
							ImGui::GetForegroundDrawList()->AddLine(ImVec2(globals->width / 2, globals->height / 2), ImVec2(hitbox_screen.x, hitbox_screen.y), ImColor(255, 255, 255, 255), 5);
						}
